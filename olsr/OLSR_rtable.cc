...
///
//THis is an empty initialization of OLSR ROuting Table;
//Empty constructor
...
//How come the iterator is not till end
OLSR_rtable::OLSR_rtable(OLSR_rtable *rtable)
{
    for (rtable_t::const_iterator itRtTable = rtable->getInternalTable()->begin();itRtTable != rtable->getInternalTable()->end();++itRtTable)
    {
