...
int numTimesHelloTimerIsFired = 0;
//int b = 0;
//int c = 0;
//int d = 0;
//int x = 0;
...
OLSR_Timer::OLSR_Timer(OLSR* agent) :
        cOwnedObject("OLSR_Timer") {
    agent_ = agent;
    tuple_ = NULL;
}
...
void OLSR_Timer::removeQueueTimer() {
    ...
    for (it = agent_->timerQueuePtr->begin();
            it != agent_->timerQueuePtr->end(); it++) {
        if (it->second == this) {
...
void OLSR_Timer::resched(double time) {
    removeQueueTimer();
    agent_->timerQueuePtr->insert(
            std::pair<simtime_t, OLSR_Timer *>(simTime() + time, this));
    //if (this->isScheduled())
...
///
void OLSR_HelloTimer::expire() {
//    double now = CURRENT_TIME;
//    double simtime = par("announcedNetworks");
//    b++;
//    if (now >= par("announcedNetworks")) {
//        cout << "Total sent HELLO messages = " << b << endl;
//    }
    agent_->send_hello();
    // agent_->scheduleAt(simTime()+agent_->hello_ival_- JITTER,this);
    agent_->timerQueuePtr->insert(
            std::pair<simtime_t, OLSR_Timer *>(
                    simTime() + agent_->hello_ival() - agent_->jitter(), this));
    numTimesHelloTimerIsFired++;
}
...
/// \param e The event which has expired.
///
void OLSR_TcTimer::expire() {
    if (agent_->mprselset().size() > 0) {
//        double now = CURRENT_TIME;
//        x++;
//        if (now >= 595) {
//            cout << "Total sent TC messages = " << x << endl;
//        }
        agent_->send_tc();
    } else {
    ...
    agent_->timerQueuePtr->insert(
            std::pair<simtime_t, OLSR_Timer *>(
                    simTime() + agent_->tc_ival() - agent_->jitter(), this));
...
void OLSR_MidTimer::expire() {
    ...
    agent_->timerQueuePtr->insert(
            std::pair<simtime_t, OLSR_Timer *>(
                    simTime() + agent_->mid_ival() - agent_->jitter(), this));
#endif
}
...
/// \param e The event which has expired.
///
void OLSR_HnaTimer::expire() {
    if ((agent_->associations().size() > 0)) {
//        double now = CURRENT_TIME;
//        WATCH(now);
//        a++;
//        if (now >= 595) {
//            cout << "Total sent HNA messages = " << a << endl;
//        }
        agent_->send_hna();
    } else {
    ...
    agent_->timerQueuePtr->insert(
            std::pair<simtime_t, OLSR_Timer *>(
                    simTime() + agent_->hna_ival() - agent_->jitter(), this));
...
void OLSR_DupTupleTimer::expire() {
    ...
    if (time < SIMTIME_DBL(simTime())) {
        ...
        agent_->timerQueuePtr->insert(
                std::pair<simtime_t, OLSR_Timer *>(simTime() + DELAY_T(time),
                        this));
    }
}
...
OLSR_LinkTupleTimer::OLSR_LinkTupleTimer(OLSR* agent, OLSR_link_tuple* tuple) :
        OLSR_Timer(agent) {
    tuple_ = tuple;
...
void OLSR_LinkTupleTimer::expire() {
    double now;
    ...
    if (tuple->time() < now) {
        ...
        agent_->timerQueuePtr->insert(
                std::pair<simtime_t, OLSR_Timer *>(
                        simTime() + DELAY_T(tuple->time()), this));
    } else {
        // agent_->scheduleAt (simTime()+DELAY_T(MIN(tuple_->time(), tuple_->sym_time())),this);
        agent_->timerQueuePtr->insert(
                std::pair<simtime_t, OLSR_Timer *>(
                        simTime()+DELAY_T(MIN(tuple->time(), tuple->sym_time())),
                        this));
    }
}
...
void OLSR_Nb2hopTupleTimer::expire() {
...
void OLSR_MprSelTupleTimer::expire() {
...
void OLSR_TopologyTupleTimer::expire() {
...
void OLSR_IfaceAssocTupleTimer::expire() {
    OLSR_iface_assoc_tuple* tuple =
            dynamic_cast<OLSR_iface_assoc_tuple*>(tuple_);
    double time = tuple->time();
    if (time < SIMTIME_DBL(simTime())) {
        removeTimer();
        delete this;
...
void OLSR_AssociationTupleTimer::expire() {
    OLSR_association_tuple* tuple =
            dynamic_cast<OLSR_association_tuple*>(tuple_);
    double time = tuple->time();
    if (time < SIMTIME_DBL(simTime())) {
        removeTimer();
        delete this;
...
void OLSR::initialize(int stage) {
    ...
    if (stage == 4) {
        ...
        printOLSRmessages = (bool)par("printOLSRmessages");
        ...
        for (int i = 0; i < getNumWlanInterfaces(); i++) {
            ...
            olsr_if->wifi_if = true;
            olsr_if->if_active = true;
            if (isInMacLayer()) {
                olsr_if->address = ManetAddress(iEntry->getMacAddress());
                olsr_if->broad = ManetAddress(MACAddress::BROADCAST_ADDRESS);
            } else {
                olsr_if->address = ManetAddress(
                        iEntry->ipv4Data()->getIPAddress());
                olsr_if->broad = ManetAddress(IPv4Address::ALLONES_ADDRESS);
            }
        ...
        while ((token = tokenizer.nextToken()) != NULL) {
            std::vector<std::string> addrPair =
                    cStringTokenizer(token, "/").asVector();
            if (addrPair.size() != 2)
                throw cRuntimeError(
                        "invalid 'announcedNetworks' parameter content: '%s'",
                        token);

            IPv4Address addr = IPvXAddressResolver().resolve(
                    addrPair[0].c_str()).get4();
            IPv4Address mask = IPvXAddressResolver().resolve(
                    addrPair[1].c_str(), IPvXAddressResolver::ADDR_MASK).get4();
            addr.doAnd(mask);

            OLSR_association* tuple = new OLSR_association;
            tuple->setNet_addr(nsaddr_t(addr));
            tuple->setNetmask(nsaddr_t(mask));

            // add to HNA:
            debug("%f: Node %s adds association tuple: net_addr = %s netmask = %s\n",
            CURRENT_TIME,
            getNodeId(ra_addr()),
            addr.str().c_str(),
            mask.str().c_str());
            ...
            //silas my input ..... other way to clear OLSR ROUTING TABLES


//             if (CURRENT_TIME >= 0) {
//                 purgeTables = new cMessage("routeClear_timer");
//                     scheduleAt(CURRENT_TIME + truncnormal(200, 8), purgeTables);
//                     debug("%f: Node %s creates routing table purge event \n",
//                             CURRENT_TIME,
//                             getNodeId(ra_addr()));
//                 }
        }
        ...
        //statistics
        numHelloMsgs = 0;
//        numTimesHelloTimerIsFired = 0;
        numTcMsgs = 0;
        numHnaMsgs = 0;
        numForwTc = 0;
        numForwHna = 0;
        TotalMessagesOLSRVector = 0;

        //initWatches
        WATCH(numHelloMsgs);
//        WATCH(numTimesHelloTimerIsFired);
        WATCH(numTcMsgs);
        WATCH(numHnaMsgs);
        WATCH(numForwTc);
        WATCH(numForwHna);
        WATCH(TotalMessagesOLSRVector);
    }
...
unsigned char OLSR::get_gw_class(int down, int up) {
...
void OLSR::get_gw_speeds(unsigned char gw_class, int *down, int *up) {
...
OLSR::check_packet(cPacket* msg, nsaddr_t &src_addr, int &index) {
    ...
    if (isInMacLayer()) {
        if (!dynamic_cast<OLSR_pkt *>(msg)) // Check if olsr packet
        {
            ...
            if (op->reduceFuncionality()
                    && par("reduceFuncionality").boolValue()) {
                delete msg;
                return NULL;
            }
...
/// \param p received packet.
///
void OLSR::recv_olsr(cMessage* msg) {
...
//            double now = CURRENT_TIME;
            ...
            //TODO .... DOES IT MAKE SENSE TO HAVE THESE HERE OR IS THERE SOME PLACE ELSE IT ALREADY IS ?
            if ((msg.msg_type() == OLSR_HNA_MSG) /*|| (msg.msg_type() == 004)*/)
                {numForwHna++;}
            if ((msg.msg_type() == OLSR_TC_MSG) /*|| (msg.msg_type() == 002)*/)
                {numForwTc++;}
            if (msg.msg_type() != OLSR_HELLO_MSG)
...
void OLSR::CoverTwoHopNeighbors(const nsaddr_t &neighborMainAddr,
        nb2hopset_t & N2) { // first gather all 2-hop neighbors to be removed
...
void OLSR::mpr_computation() {
    ...
    // (iii) all the symmetric neighbors: the nodes for which there exists a symmetric
    //       link to this node on some interface.
    for (nb2hopset_t::iterator it = nb2hopset().begin();
            it != nb2hopset().end(); it++) {
        OLSR_nb2hop_tuple* nb2hop_tuple = *it;
        // (ii)  the node performing the computation
    ...
    for (nb2hopset_t::iterator it = N2.begin(); it != N2.end(); it++) {
        ...
        for (nb2hopset_t::const_iterator it2 = N2.begin(); it2 != N2.end();
                it2++) {
            OLSR_nb2hop_tuple* otherTwoHopNeigh = *it2;
            if (otherTwoHopNeigh->nb2hop_addr() == twoHopNeigh->nb2hop_addr()
                    && otherTwoHopNeigh->nb_main_addr()
                            != twoHopNeigh->nb_main_addr()) {
                onlyOne = false;
                break;
            }
        }
        if (onlyOne) {
            ...
            for (nb2hopset_t::const_iterator it2 = N2.begin(); it2 != N2.end();
                    it2++) {
                OLSR_nb2hop_tuple* otherTwoHopNeigh = *it2;
                if (otherTwoHopNeigh->nb_main_addr()
                        == twoHopNeigh->nb_main_addr()) {
                    coveredTwoHopNeighbors.insert(
                            otherTwoHopNeigh->nb2hop_addr());
                }
            }
        }
    }
    ...
    for (nb2hopset_t::iterator it = N2.begin(); it != N2.end();) {
        OLSR_nb2hop_tuple* twoHopNeigh = *it;
        if (coveredTwoHopNeighbors.find(twoHopNeigh->nb2hop_addr())
    ...
    while (N2.begin() != N2.end()) {
        ...
        for (nbset_t::iterator it = N.begin(); it != N.end(); it++) {
            OLSR_nb_tuple* nb_tuple = *it;
            int r = 0;
            for (nb2hopset_t::iterator it2 = N2.begin(); it2 != N2.end();
                    it2++) {
                OLSR_nb2hop_tuple* nb2hop_tuple = *it2;
        ...
        for (std::set<int>::iterator it = rs.begin(); it != rs.end(); it++) {
            ...
            for (std::vector<OLSR_nb_tuple *>::iterator it2 =
                    reachability[r].begin(); it2 != reachability[r].end();
                    it2++) {
                OLSR_nb_tuple *nb_tuple = *it2;
                if (max == NULL
...
void OLSR::rtable_computation() {
    ...
    nsaddr_t next(IPv4Address::ALLONES_ADDRESS);

    /// 1. All the entries from the routing table are removed.
    //

    if (par("DelOnlyRtEntriesInrtable_").boolValue()) {
        for (rtable_t::const_iterator itRtTable =
                rtable_.getInternalTable()->begin();
                itRtTable != rtable_.getInternalTable()->begin(); ++itRtTable) {
        ...
        if(par("isFlooding")){
            ...
            if(associations().empty() && (getInterfaceTable()->getNumInterfaces() <= 2)){
                omnet_clean_rte();// clean IP tables
            }if(!associations().empty() && (getInterfaceTable()->getNumInterfaces() > 2)){
                // clean the route table wlan0 interface entry
                omnet_partial_clean_rte();
            }
        }
    }
    rtable_.clear();
    ...
    for (nbset_t::iterator it = nbset().begin(); it != nbset().end(); it++) {
        ...
        if (nb_tuple->getStatus() == OLSR_STATUS_SYM) {
            ...
            if (!nb_main_addr && lt != NULL) {
                rtable_.add_entry(nb_tuple->nb_main_addr(), lt->nb_iface_addr(),
                        lt->local_iface_addr(), 1, lt->local_iface_index());

                if (!useIndex)
                    omnet_chg_rte(nb_tuple->nb_main_addr(), lt->nb_iface_addr(),
    ...
    for (nb2hopset_t::iterator it = nb2hopset().begin();
        ...
        if (nb_tuple == NULL)
            ...
            if (nb_tuple != NULL)
                ...
                nb_tuple = state_.find_sym_nb_tuple(
        ...
        if (ok) {
            ...
            bool insert = true;
            OLSR_rt_entry* entry2hop = rtable_.lookup(
                    nb2hop_tuple->nb2hop_addr());
            if (entry2hop) {
                // check if the node is a better alternative
                OLSR_nb_tuple* nb_tupleNew = state_.find_sym_nb_tuple(
                        nb2hop_tuple->nb_main_addr());
                OLSR_nb_tuple* nb_tupleOld = state_.find_sym_nb_tuple(
                        entry2hop->next_addr());

                if (nb_tupleOld != NULL
                        && nb_tupleOld->willingness()
                                > nb_tupleNew->willingness())
                    insert = false;
            }
            ...
            if (insert) {
                rtable_.add_entry(nb2hop_tuple->nb2hop_addr(),
                        ...
                        entry->local_iface_index());
                if (!useIndex)
                    omnet_chg_rte(nb2hop_tuple->nb2hop_addr(),
                            entry->next_addr(), netmask, 2, false,
                            entry->iface_addr());
                else
                    omnet_chg_rte(nb2hop_tuple->nb2hop_addr(),
                            entry->next_addr(), netmask, 2, false,
        ...
        for (topologyset_t::iterator it = topologyset().begin();
            ...
            if (entry1 == NULL && entry2 != NULL && entry2->dist() == h) {
                rtable_.add_entry(topology_tuple->dest_addr(),
                        ...
                        entry2->local_iface_index(), entry2);

                if (!useIndex)
                    omnet_chg_rte(topology_tuple->dest_addr(),
                            entry2->next_addr(), netmask, h + 1, false,
                            entry2->iface_addr());

                else
                    omnet_chg_rte(topology_tuple->dest_addr(),
                            entry2->next_addr(), netmask, h + 1, false,
                            entry2->local_iface_index());
        ...
        for (ifaceassocset_t::iterator it = ifaceassocset().begin();
            ...
            if (entry1 != NULL && entry2 == NULL) {
                rtable_.add_entry(tuple->iface_addr(), entry1->next_addr(),
                        ...
                        entry1->local_iface_index(), entry1);

                if (!useIndex)
                    omnet_chg_rte(tuple->iface_addr(), entry1->next_addr(),
                            netmask, entry1->dist(), false,
                            entry1->iface_addr());

                else
                    omnet_chg_rte(tuple->iface_addr(), entry1->next_addr(),
                            netmask, entry1->dist(), false,
                            entry1->local_iface_index());
                added = true;
            }
        }
    ...
    if (rtable_.size() != prev_rtablesize) { //only log routing table size changes.
        std::cout << "OLSR: routing_table_size: " << ra_addr() << " "
                << rtable_.size() << " " << simTime() << endl;
    ...
    for (associationset_t::iterator it = associationset().begin();
        ...
        OLSR_rt_entry* entry = rtable_.lookup(tuple->net_addr());
        OLSR_rt_entry* gwEntry = rtable_.lookup(tuple->gateway());
        if (gwEntry == NULL)
            break;
        // (a.)Test if HNA associations received from other gateways
        // are also announced by this node. In such a case, no route
        // is created for this association tuple (go to the next one).
        // (b.)check if the entry is already in the routing table ... ELSE, ADD ENTRY
        if ((entry == NULL) && (ra_addr() != tuple->gateway())) {
            rtable_.add_entry(tuple->net_addr(), gwEntry->next_addr(),
                    gwEntry->iface_addr(), gwEntry->dist(),
                    gwEntry->local_iface_index(), gwEntry);
            if (!useIndex)
                omnet_chg_rte(tuple->net_addr(), gwEntry->next_addr(),
                        tuple->netmask(), gwEntry->dist(), false,
                        gwEntry->iface_addr());
            else
                omnet_chg_rte(tuple->net_addr(), gwEntry->next_addr(),
                        tuple->netmask(), gwEntry->dist(), false,
                        gwEntry->local_iface_index());

            added = true;
        }
        if ((entry != NULL) && (ra_addr() != tuple->gateway())) {
            if(entry->dist() >= gwEntry->dist()){
            rtable_.add_entry(tuple->net_addr(), gwEntry->next_addr(),
                    gwEntry->iface_addr(), gwEntry->dist(),
                    gwEntry->local_iface_index(), gwEntry);
            if (!useIndex)
    ...
    //THIS ADDS A DEFAULT ROUTE TO THE ROUTING TABLE OF A GATEWAY NODE .....

        // TODO: could be modified
        int interfaces = getNumInterfaces();
        if ((interfaces >= 3) && par("isFlooding")) {
            nsaddr_t addr1(getIfaceAddressFromIndex(2));
            for (associations_t::iterator it = associations().begin();
                ...
                if ((entry == NULL)) {
                    ...
                    rtable_.add_entry(tuple->net_addr(), next, getRouterId(), 1,
                            1);
                    if (!useIndex)
                        omnet_chg_rte(tuple->net_addr(), next, tuple->netmask(), 1,
                                false, 2);
                    else
                        omnet_chg_rte(tuple->net_addr(), addr1, tuple->netmask(), 1,
                                false, 1);
    ...
    //MY INPUT ...... (FOR DHT UPDATING).... application spesific
    if(!associations().empty() && (getInterfaceTable()->getNumInterfaces() > 2) && par("isMobility")){
        cache_recent_table();
    }
}
...
///
bool OLSR::process_hello(OLSR_msg& msg, const nsaddr_t &receiver_iface,
        const nsaddr_t &sender_iface, const int &index) {
    /*f( sender_iface.compare(getIfaceAddressFromIndex(1)) != receiver_iface.compare(getIfaceAddressFromIndex(1)))
     {
     return false;}*/
...
/// \param sender_iface the address of the interface where the message was sent from.
///
bool OLSR::process_tc(OLSR_msg& msg, const nsaddr_t &sender_iface,
        const int &index) {
...
/// \param sender_iface the address of the interface where the message was sent from.
///
void OLSR::process_mid(OLSR_msg& msg, const nsaddr_t &sender_iface,
        const int &index) {
...
bool OLSR::process_hna(OLSR_msg& msg, const nsaddr_t &sender_iface,
        const int &index) {
    ...
    // 1. If the sender interface of this message is not in the symmetric
    // 1-hop neighborhood of this node, the message MUST be discarded
    //(According to RFC 3626 Specification):
    OLSR_link_tuple* link_tuple = state_.find_sym_link_tuple(sender_iface, now);
    if (link_tuple == NULL)
        return false;

    // 2. Otherwise, For each (network address, netmask) pair received in
    // the HNA message (According to RFC 3626 Specification):
    for (int i = 0; i < hna.count; i++) {
        assert(i >= 0 && i < OLSR_MAX_ADDRS);
        nsaddr_t addr = hna.net_addr(i);
        nsaddr_t ra_addr = msg.orig_addr();
        nsaddr_t netmask = hna.netmask(i);

        //gatway nodes are not meant to add default routes for a HNA message.
        //a HNA route will only be added if it is not a default route.
        if(!associations().empty())
        {
            if(msg.hna().net_addr(i).isUnspecified())
                return false;
        }

        // 2.1. If there exist some entry in the association set where:
        //  A_gateway_addr  == message originator address
        //  A_network_addr  == Destination network address AND
        //  A_netmask       == Destination network mask
        // then the holding time of that tuple MUST be set to:
        ...
        if (tuple != NULL)
            tuple->time() = now + OLSR::emf_to_seconds(msg.vtime());
        // 2.2. Otherwise, a new tuple MUST be recorded in the association
        // set where:
        //  A_gateway_addr  == message originator address
        //  A_network_addr  == Destination network address AND
        //  A_netmask       == Destination network mask
        //  A_time          ==  current time + validity time.
            ...
            tuple->setNet_addr(addr);
            tuple->setGateway(ra_addr);
            tuple->setNetmask(netmask);
            tuple->time() = now + OLSR::emf_to_seconds(msg.vtime());
            add_association_tuple(tuple);
            EV << "i just received an HNA MESSAGE!!!!" << endl;
            std::string tempString { "received HNA message (" };
            tempString.append(tuple->net_addr().str());
            tempString.append(" @ ");
            tempString.append(ra_addr.str());
            tempString.append(")");
            getParentModule()->bubble(tempString.c_str());
...
///
void OLSR::forward_default(OLSR_msg& msg, OLSR_dup_tuple* dup_tuple,
        const nsaddr_t &local_iface, const nsaddr_t &src_addr) {
    double now = CURRENT_TIME;
...
void OLSR::send_pkt() {
    ...
    int num_pkts =
            (num_msgs % OLSR_MAX_MSGS == 0) ?
                    num_msgs / OLSR_MAX_MSGS : (num_msgs / OLSR_MAX_MSGS + 1);
    ...
    /*for(int it = 0 ; it <= num_msgs; it++){

     }*/

    for (int i = 0; i < num_pkts; i++) {
        OLSR_pkt* op = new OLSR_pkt;
        if (msgs_.begin()->msg_type_ == 4) {
            op->setName("HNA MSG");
        } else {
            op->setName("OLSR Pkt");
        }
        ...
        int j = 0;
        for (std::vector<OLSR_msg>::iterator it = msgs_.begin();
                it != msgs_.end();) {
            if (j == OLSR_MAX_MSGS)
                break;
        ...
        sendToIp(op, RT_PORT, destAdd, RT_PORT, IP_DEF_TTL, 0.0,
                ManetAddress::ZERO);
    }
}
...
void OLSR::send_hello() {
    ...
    for (linkset_t::iterator it = linkset().begin(); it != linkset().end();
            it++) {
        OLSR_link_tuple* link_tuple = *it;
        if (get_main_addr(link_tuple->local_iface_addr()) == ra_addr()
                && link_tuple->time() >= now) {
            ...
            if (use_mac() && link_tuple->lost_time() >= now)
                ...
                link_type = OLSR_LOST_LINK;
            EV << "SENDING HELLO !!!!" << endl;
            // Establishes neighbor type.
            if (state_.find_mpr_addr(
                    get_main_addr(link_tuple->nb_iface_addr())))
            ...
            std::map<uint8_t, int>::iterator pos = linkcodes_count.find(
                    link_code);
...
void OLSR::send_tc() {
    ...
    msg.tc().reserved() = 0;
    msg.tc().count = 0;
    //    const simtime_t a = SIMTIME_DBL(simTime());

    for (mprselset_t::iterator it = mprselset().begin();
            it != mprselset().end(); it++) {
    ...
    enque_msg(msg, JITTER);
    numTcMsgs++;
    //recordScalar("Total TC", numTcMsgs++);
}
...
void OLSR::send_hna() {
    ...
    msg.vtime() = OLSR::seconds_to_emf(OLSR_HNA_HOLD_TIME);
    msg.orig_addr() = ra_addr();
    for (int i = 0; i <= getNumWlanInterfaces(); i++) {
        msg.iface_addr() = getIfaceAddressFromIndex(i);
    }
    msg.ttl() = 255;
    msg.hop_count() = 0;
    msg.msg_seq_num() = msg_seq();
    ...
    for (associations_t::iterator it = associations().begin();
        ...
        assert(count >= 0 && count < OLSR_MAX_ADDRS);
        msg.hna().iface_addr(count) = ra_addr();    //, tuple->netmask();
        msg.hna().net_addr(count) = tuple->net_addr();
        msg.hna().netmask(count) = tuple->netmask();
        for (int i = 0; i < getNumWlanInterfaces(); i++) {
            int index = getWlanInterfaceIndex(i);
            nsaddr_t addr = getIfaceAddressFromIndex(index);
            msg.hna().setIface_addr(i, addr);
        }
        msg.hna().count++;
        EV << "hna message is : " << msg.hna().net_addr(count).str() << "/"
                  << msg.hna().netmask(count) << endl;
        EV << "sender interface is : " << msg.hna().iface_addr(0) << endl;
        if (debugOutput) {
            EV << "HNA MESSAGE" << endl;
        }
    ...
    enque_msg(msg, JITTER);
    numHnaMsgs++;
//    recordScalar("Total HNA", numHnaMsgs++);
    // If there is no HNA associations to send, return without queuing the message
    if (associations().size() == 0) {
        EV << "empty association set: message not queued" << endl;
        return;
    }

}

//adds all network addresses and prefixes to the association set of a gateway node
///(following RFC 3626 specification)

void OLSR::add_association(OLSR_association* tuple) {
    // Check if the (networkAddr, netmask) tuple already exist
    // in the list of HNA associations
    for (associations_t::iterator it = associations().begin();
            it != associations().end(); it++) {
        if (*it == tuple) {
            return;
    ...
    // If the tuple does not already exist, add it to the list of HNA associations.
    state_.insert_association(tuple);
}

void OLSR::rm_association(OLSR_association* tuple) {
...
bool OLSR::link_sensing(OLSR_msg& msg, const nsaddr_t &receiver_iface,
        const nsaddr_t &sender_iface, const int &index) {
...
/// \param msg the %OLSR message which contains the HELLO message.
///
bool OLSR::populate_nbset(OLSR_msg& msg) {
    OLSR_hello& hello = msg.hello();
...
/// \param msg the %OLSR message which contains the HELLO message.
///
bool OLSR::populate_nb2hopset(OLSR_msg& msg) {
    double now = CURRENT_TIME;
    OLSR_hello& hello = msg.hello();

    for (linkset_t::iterator it_lt = linkset().begin();
            it_lt != linkset().end(); it_lt++) {
        ...
        if (get_main_addr(link_tuple->nb_iface_addr()) == msg.orig_addr()) {
            if (link_tuple->sym_time() >= now) {
                ...
                for (int i = 0; i < hello.count; i++) {
                    ...
                    int nt = hello_msg.link_code() >> 2;
                    assert(
                            hello_msg.count >= 0 && hello_msg.count <= OLSR_MAX_ADDRS);

                    for (int j = 0; j < hello_msg.count; j++) {
                        nsaddr_t nb2hop_addr = get_main_addr(
                                hello_msg.nb_iface_addr(j));
                        if (nt == OLSR_SYM_NEIGH || nt == OLSR_MPR_NEIGH) {
                            ...
                            if (nb2hop_addr != ra_addr()) {
                                ...
                                state_.find_nb2hop_tuple(msg.orig_addr(),
                                        nb2hop_addr);
...
///
void OLSR::mac_failed(IPv4Datagram* p) {
    double now = CURRENT_TIME;

    nsaddr_t dest_addr = ManetAddress(p->getDestAddress());

    EV << "Node " << OLSR::node_id(ra_addr())
              << "MAC Layer detects a breakage on link to "
    ...
    if (entry) {
        OLSR_link_tuple* link_tuple = state_.find_link_tuple(
                entry->next_addr());
        if (link_tuple != NULL) {
...
///
void OLSR::add_link_tuple(OLSR_link_tuple* tuple, uint8_t willingness) {
    double now = CURRENT_TIME;
...
///
void OLSR::updated_link_tuple(OLSR_link_tuple* tuple, uint8_t willingness) {
    double now = CURRENT_TIME;
...
OLSR_nb_tuple* OLSR::find_or_add_nb(OLSR_link_tuple* tuple,
        uint8_t willingness) {
    OLSR_nb_tuple* nb_tuple = state_.find_nb_tuple(
            get_main_addr(tuple->nb_iface_addr()));
    if (nb_tuple == NULL) {
        double now = CURRENT_TIME;
...
///
/// \brief Adds an association tuple to the association Set.
///
/// \param tuple the association tuple to be added.
///

void OLSR::add_association_tuple(OLSR_association_tuple* tuple) {
    debug("%f: Node %s adds association tuple: net_addr = %s netmask = %s gateway = %s\n",
    CURRENT_TIME,
    getNodeId(ra_addr()),
    tuple->net_addr().str().c_str(),
    tuple->netmask().str().c_str(),
    tuple->gateway().str().c_str());
...
///
/// \brief Removes an association tuple from the association Set.
///
/// \param tuple the association tuple to be removed.
///

void OLSR::rm_association_tuple(OLSR_association_tuple* tuple) {
    debug("%f: Node %s removes association tuple: net_addr = %s netmask = %s\n",
    CURRENT_TIME,
    getNodeId(ra_addr()),
    tuple->net_addr().str().c_str(),
    tuple->netmask().str().c_str(),
...
///
bool OLSR::seq_num_bigger_than(uint16_t s1, uint16_t s2) {
...
///
uint8_t OLSR::seconds_to_emf(double seconds) {
    ...
    if (b < 0) {
        ...
        a =
                (int) (16
                        * ((double) seconds
                                / (OLSR_C * (double) pow((double) 2, b)) - 1));
        while (a >= 16) {
            a -= 16;
            b++;
        }
...
///
double OLSR::emf_to_seconds(uint8_t olsr_format) {
    // This implementation has been taken from unik-olsrd-0.4.5 (mantissa.c),
    // licensed under the GNU Public License (GPL)
    int a = olsr_format >> 4;
    int b = olsr_format - a * 16;
    return (double) (OLSR_C * (1 + (double) a / 16)
            * (double) pow((double) 2, b));
}
...
///
int OLSR::node_id(const nsaddr_t &addr) {
    return addr.getIPv4().getInt();
    /*
     // Preventing a bad use for this function
     if (nsaddr_t)addr == IP_BROADCAST)
     return addr;
     // Getting node id
...
void OLSR::processLinkBreak(const cObject *details) {
    if (use_mac()) {
        if (dynamic_cast<IPv4Datagram *>(const_cast<cObject*>(details))) {
            IPv4Datagram * dgram = const_cast<IPv4Datagram *>(check_and_cast<
...
void OLSR::finish() {
    ...
    // My input: the tracking is modified to track the total messages sent by module and globally
        recordScalar("OLSR_HNA_Messages_(Forwarded)", numForwHna);
       /* globalStatistics->recordHistogram ("OLSR: Total HNA Messages ",
                numForwHna);*/

        recordScalar("OLSR_TC_Messages_(Forwarded)", numForwTc);
        /*globalStatistics->recordHistogram ("OLSR: TC Messages (Forwarded) ",
                         numForwTc);*/

        recordScalar("OLSR_Total_HELLO", numHelloMsgs);
        /*globalStatistics->recordHistogram ("OLSR: Total HELLO ",
         *
         */
        recordScalar("OLSR_Total_HNA_(originated)", numHnaMsgs);

        recordScalar("OLSR_Total_TC", numTcMsgs);
        /*globalStatistics->recordHistogram ("OLSR: Total TC ",
                numTcMsgs);*/

        TotalMessagesOLSRVector = numTcMsgs + numHelloMsgs + numForwTc + numForwHna + numHnaMsgs;

        recordScalar("OLSR_Total_Messages_Sent", TotalMessagesOLSRVector);

        if (numTcMsgs==0)
           {
               if (printOLSRmessages)
                   EV << getFullPath() << ": No TC message sent, skipping recording statistics and printing results.\n";
           }
        if (numHelloMsgs==0)
           {
               if (printOLSRmessages)
                   EV << getFullPath() << ": No Hello messages sent, skipping recording statistics and printing results.\n";
           }
        if (numHnaMsgs==0)
           {
               if (printOLSRmessages)
                   EV << getFullPath() << ": No HNA messages sent, skipping recording statistics and printing results.\n";
           }
        ...
        if (printOLSRmessages)
           {
               ...
               cout << "--------------------------------------------------------" << endl;
               cout <<getFullPath()<< "_OLSR_Hello: " << numHelloMsgs <<endl;
               cout <<getFullPath()<< "_OLSR_Hello_timer: " << numTimesHelloTimerIsFired <<endl;
               cout <<getFullPath()<< "_OLSR_TC: " << numTcMsgs <<endl;
               cout <<getFullPath()<< "_OLSR_HNA: " << numHnaMsgs <<endl;
               cout <<getFullPath()<< "_OLSR_Forwarded_TC: " << numForwTc <<endl;
               cout <<getFullPath()<< "_OLSR_Forwarded_HNA: " << numForwHna <<endl;
               cout <<getFullPath()<< "_OLSR_Total_control_messages: " << TotalMessagesOLSRVector <<endl;
               cout << "--------------------------------------------------------" << endl;
...
// ----- silas, my input ........
// make copies of entries in the routing table with interface wlan0
//
void OLSR::cache_recent_table()
{
    if(CURRENT_TIME > (double)par("Hello_ival")*40){

        IPv4Route *entry;
        inet_rt = RoutingTableAccess().getIfExists();
        nb = NotificationBoardAccess().get();
        bool found;
//        IPv4Address entryToDelete;
//        int count = 0;
        IPv4Address address = IPv4Address::UNSPECIFIED_ADDRESS;

        for (rTable_CacheVector::iterator it = item().begin(); it != item().end(); it++) {
            rTable_Cache prevItem = *it;
            for (int i=inet_rt->getNumRoutes()-1; i>=0; i--)
            {
                found = false;
                entry = inet_rt->getRoute(i);
                if ((strcmp(entry->getInterfaceName() , "wlan0") == 0) && (!entry->getInterface()->isLoopback()))
                {
                    if (prevItem.dest_addr() == entry->getDestination()) {
                        found = true;
                        break;
                    }
                }
            }
            if(!found && (prevItem.interface() == 0)){
                IPv4Route *deleteCtrl = new IPv4Route;
                deleteCtrl->setGateway(prevItem.gw_addr());
                deleteCtrl->setDestination(prevItem.dest_addr());
//                entryToDelete = prevItem.dest_addr();
                nb->fireChangeNotification(NF_IPv4_ROUTE_REMOVED, deleteCtrl);
                delete deleteCtrl;
//                it = item().erase(item().begin()+count);
//                count--;
            }
//            count++;
        }
...
//        }
        ...
        for (int i=inet_rt->getNumRoutes()-1; i>=0; i--)
        {
            ...
            if ((strcmp(entry->getInterfaceName() , "wlan0")== 0) && (!entry->getInterface()->isLoopback()))
            {
                ...
                for (rTable_CacheVector::iterator it = item().begin(); it != item().end(); it++) {
                    rTable_Cache prevItem = *it;
                    if(entry->getDestination() == prevItem.dest_addr()){
                        found = true;
                        break;
                    }
                }
                if(found == false){
                    rTable_Cache newCache;
                    newCache.setDest_addr(entry->getDestination());
                    newCache.setGw_addr(entry->getGateway());
                    newCache.setNetmask(entry->getNetmask());
                    newCache.setHopCount(entry->getAdminDist());
                    newCache.setInterface(0);
                    item().push_back(newCache);
                }
            }
        }
//        return;
...
uint32_t OLSR::getRoute(const ManetAddress &dest,
...
bool OLSR::getNextHop(const ManetAddress &dest, ManetAddress &add, int &iface,
        double &cost) {
    OLSR_rt_entry* rt_entry = rtable_.lookup(dest);
    if (!rt_entry) {
        ManetAddress apAddr;
...
int OLSR::getRouteGroup(const AddressGroup &gr,
        std::vector<ManetAddress> &add) {
    ...
    for (AddressGroupConstIterator it = gr.begin(); it != gr.end(); it++) {
        ...
        if (!rt_entry)
            continue;
        if (distance < (int) rt_entry->dist()
                || (distance == (int) rt_entry->dist() && intrand(1)))
            continue;
...
bool OLSR::getNextHopGroup(const AddressGroup &gr, ManetAddress &add,
        int &iface, ManetAddress &gw) {
...
int OLSR::getRouteGroup(const ManetAddress& dest,
        std::vector<ManetAddress> &add, ManetAddress& gateway, bool &isGroup,
        int group) {
    AddressGroup gr;
    int distance = 0;
...
bool OLSR::getNextHopGroup(const ManetAddress& dest, ManetAddress &next,
        int &iface, ManetAddress& gw, bool &isGroup, int group) {
    AddressGroup gr;
    bool find = false;
    if (findInAddressGroup(dest, group)) {
        getAddressGroup(gr, group);
...
void OLSR::getDistributionPath(const ManetAddress &addr,
...
bool OLSR::isNodeCandidate(const nsaddr_t &src_addr) {
    double now = CURRENT_TIME;
